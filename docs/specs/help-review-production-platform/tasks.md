...
- [ ] 1.1 Confirm the scientist scoring contract (2-4 hours)
  ...
  - **Status (July 15):** `help-scoring-v0`, deterministic fixtures, fake/Gemini/Vertex adapters, and the contract-compatible `evidence-first-v1` reference scorer are implemented. The reference path separates timestamped observation from candidate adjudication, validates exact event/skill allowlists, and safely gates confidence and opportunity-dependent credits. Closure still requires Yi's versioned package/data contract and accepted fixtures, plus an educator-labelled comparison set before selecting either scorer for real-data use (D-001, D-002, D-015, D-017).
...
- [ ] 1.4 Confirm HELP content and final-output contracts (2-4 hours)
  ...
  - **Status (July 15):** `help-catalog-v1` now imports and validates immutable skill identity/code/order, hierarchy labels, age ranges, provenance, observable indicators/non-examples, observation conditions, evidence modality, video scoreability, all four credit definitions, selection policy, and the two-minus decision reference. Both images package the selected artifact, assessments snapshot its version, and real-data mode rejects the fixture and the new source-attributed 127-item CDC/ELOF research corpus. Closure requires the owner-issued authoritative HELP artifact and final-output mapping (D-003-D-005, D-009-D-011).
