...
The implementation has passed TypeScript, ESLint, 44 focused Vitest checks, Prisma validation, an optimized Next.js build, direct authorization/permission requests, and interactive Playwright checks across desktop, tablet, and 360 px mobile states. The deterministic local state is recreated from `lib/help-review/fixtures.ts`; `.data/` remains ignored.

Unchecked boxes are intentionally retained wherever the task's completion criteria require an approved external contract, organization-owned infrastructure, provider contract tests, staging evidence, stakeholder approval, or a committed 45-state regression baseline. The local runtime does not turn those unresolved launch gates into rejected features, and production startup rejects the sanitized adapters unless an explicit sanitized-demonstration acknowledgement is present.
