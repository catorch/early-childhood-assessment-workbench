...
The checked-in `content/help-catalog.sanitized.json` is an eight-skill engineering fixture, not the authoritative catalogue. Real-data mode rejects it.
...
| `status` | `AUTHORITATIVE` for real-data use; `SANITIZED_FIXTURE` otherwise |
...
The processor sends only the age/support-context-selected candidate projection: stable ID, code, name, domain, strand, inclusive age range, and source order. Credit definitions and selection policy remain application-owned inputs. Model output is cross-validated against the exact immutable request, so a result cannot invent or rename a skill, change its order, or cite evidence outside the video duration.
