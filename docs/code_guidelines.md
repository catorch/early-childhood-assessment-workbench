# Engineering Guidelines (TypeScript)

## North star: Don’t mask bugs we control
Primary objective: **ship the smallest correct change** that improves reliability and readability — **without** permissive fallbacks, silent error swallowing, or “AI slop” bloat.

## Non‑negotiables
1. **No silent exception swallowing**
2. **No permissive fallbacks for controlled inputs**
3. **Fail fast on first‑party data**
4. **Keep code minimal**

## Overview
WIP document describing the engineering principles for this codebase. Style guides are useful, but this is a principles-level doc — don’t cargo-cult a checklist.

## General principles
Generally speaking, readability is about helping the reader understand a part of the system while minimizing the amount of extra system context they need to pull in. Keep this in mind to create good code from first principles instead of memorizing rules.

## Documentation (TSDoc / JSDoc)
As a rule of thumb, non-trivial modules (files) should have a short header comment, and exported functions/types should have TSDoc unless they’re trivial and fully described by their name and signature.

- Always document side effects and non-parameter inputs (network calls, `localStorage` keys, environment variables/build-time flags, files, etc.).
- Always document input expectations. It’s okay (and good) for functions to be picky about what they accept, but this must be documented.
- Remember: TypeScript types don’t exist at runtime. For untrusted inputs, document + validate the runtime shape.

## Technical debt
Our goal is to move quickly, but shortcuts can slow us down even on short timelines. If a 30-minute shortcut costs a couple of hours two weeks from now to disentangle or explain, it’s not worth it.

Some cases where it’s usually worth doing the right thing even when moving quickly:

- **Duplicating code:** almost never the right call. It’s very hard to move quickly when two parts of the code are silently coupled.
- **Overly-dynamic interfaces:** sometimes (rarely) it’s worth using dynamic shapes (e.g., `Record<string, unknown>`). If the schema is first-party and stable, prefer explicit TypeScript types instead.

## Encapsulation
Functions, classes, and modules should be as coherent as possible: they should represent a meaningful, reusable abstraction instead of being overfit to the current caller.

**Bad (IO + logic entangled + duplicated pipeline):**
```ts
export async function computeError(path1: string, path2: string): Promise<[number, number]> {
  const data1 = await load(path1);
  const data2 = await load(path2);

  const intermediate1 = someFn(data1);
  const intermediate2 = someFn(data2);

  return [otherFn(intermediate1), otherFn(intermediate2)];
}
```

**Good (pure compute is reusable; IO stays at the edges):**
```ts
function computeIncError(data: Data): number {
  return otherFn(someFn(data));
}

export async function computeErrors(paths: readonly string[]): Promise<number[]> {
  const datasets = await Promise.all(paths.map(load));
  return datasets.map(computeIncError);
}
```

There are two major problems in the “bad” example:

1) Entangling IO (loading) with compute makes the code brittle and hard to reuse (1 input vs N inputs).
2) Copy/pasting the same pipeline hard-codes coupling. If we do the same sequence of operations multiple times, express it once and iterate.

## Interfaces (TypeScript types)
- Prefer explicit `type` / `interface` definitions at module boundaries.
- Avoid `any`. For untrusted data, start at `unknown` and narrow/validate.
- Avoid “option bag” objects with a dozen optional fields when the schema is first-party; model required vs optional fields explicitly.
- Prefer discriminated unions for variants and `readonly` where mutation isn’t required.

## Input handling
For interfaces whose callers we own, do not be permissive about bad inputs. Document what you expect and validate at the top of the function (throw a clear error).

For inputs we do not own (incoming web requests, parsed JSON, third-party payloads), handle more leniently — but do it intentionally: parse to a well-typed shape and keep the raw input as `unknown`.

For first-party objects whose schema we control, do **not** use permissive access patterns to paper over missing required fields (this hides bugs and turns them into plausible-looking defaults), e.g.:

- `const value = obj.required ?? defaultValue`
- `const value = obj.required || defaultValue`

Instead, keep required fields required in the type, or validate/throw when crossing a boundary.

For first-party workflow artifacts that we expect to exist (derived datasets, generated summaries, intermediate outputs, etc.), treat absence as a bug unless the interface is explicitly documented as optional. Do not silently downgrade required outputs into `null`, “Not available”, or placeholders just to keep the workflow moving.

## Error handling
Fail loudly on logic bugs — especially when continuing would make the rest of the computation misleading. Early returns with a default value hide bugs from the rest of the program.

In TypeScript/JS specifically:

- Use `try/catch` only to add context, translate errors at boundaries, or implement truly optional behavior; otherwise rethrow.
- When wrapping errors, preserve causes (e.g. `new Error("…", { cause })`) so failures remain diagnosable.

## Style guide / tooling
- Prefer consistency with existing code in this repo.
- Before shipping, run `npm run typecheck` (strict `tsc`) and the relevant tests.
