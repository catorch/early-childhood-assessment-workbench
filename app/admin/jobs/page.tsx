...
function AdminJobsContent() {
  ...
  return (
    <PageShell>
      <header className="flex items-end justify-between gap-6 max-sm:items-start max-sm:flex-col"><div><Eyebrow>Administration</Eyebrow><h1 className="mt-1 font-heading text-4xl font-normal leading-tight max-sm:text-[30px]">Processing jobs</h1><p className="mt-2.5 leading-relaxed text-muted-foreground">Review failed pilot processing and retry only when the source video is available.</p></div><Button onClick={() => void load()} size="sm" type="button" variant="secondary"><RefreshCw aria-hidden="true" size={15} /> Refresh jobs</Button></header>
