    ...
    <PageShell>
      ...
      {provisionOpen ? (
        <form className="mt-7 grid grid-cols-[minmax(210px,1fr)_minmax(150px,.65fr)_minmax(200px,.8fr)_120px_auto] items-end gap-4 border-y border-border border-t-[3px] border-t-primary bg-surface p-5 max-lg:grid-cols-2 max-lg:[&>div:first-child]:col-span-full max-md:grid-cols-1 max-md:[&>div:first-child]:col-span-1 max-sm:p-4" id="provision-staff-form" onSubmit={provision}>
          <div><Eyebrow>Approved staff</Eyebrow><h2 className="mt-1 font-heading text-xl font-bold">Provision pilot access</h2><p className="mt-1 text-xs text-muted-foreground">{data?.identityMode === "email-password" ? "An invitation email with a secure account-setup link is sent after provisioning." : "Sanitized sandbox profiles sign in without credentials."}</p></div>
        ...
        <div className="mt-7 grid grid-cols-[minmax(280px,.7fr)_minmax(0,1.3fr)] items-start gap-7 max-md:grid-cols-1">
          ...
          {selected ? <section className="border border-border bg-surface p-5" aria-labelledby="access-editor-title">
            <header className="flex flex-wrap items-center gap-3 border-b border-border pb-5">
              ...
              <div className="min-w-0 flex-1 basis-52"><Eyebrow>{selected.role === "ADMIN" ? "Admin / Supervisor access" : "Educator access"}</Eyebrow><h2 className="mt-1 break-words font-heading text-2xl font-bold" id="access-editor-title">{selected.displayName}</h2><p className="mt-1 truncate text-sm text-muted-foreground">{selected.email}</p></div>
