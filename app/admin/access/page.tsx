...
function AdminAccessContent() {
  ...
  return (
    <PageShell>
      ...
      {provisionOpen ? (
        <form className="mt-7 grid grid-cols-[minmax(210px,1fr)_minmax(150px,.65fr)_minmax(200px,.8fr)_120px_auto] items-end gap-4 rounded-2xl border border-border bg-surface p-5 shadow-card max-lg:grid-cols-2 max-lg:[&>div:first-child]:col-span-full max-md:grid-cols-1 max-md:[&>div:first-child]:col-span-1 max-sm:p-4" id="provision-staff-form" onSubmit={provision}>
          <div><Eyebrow>Approved staff</Eyebrow><h2 className="mt-2 text-xl font-extrabold">Provision pilot access</h2><p className="mt-1 text-xs text-muted-foreground">{data?.identityMode === "email-password" ? "An invitation email with a secure account-setup link is sent after provisioning." : "Sanitized sandbox profiles sign in without credentials."}</p></div>
      ...
      {data && data.staff.length > 0 ? <>
        <div className="mt-7 flex items-center gap-4 py-1 max-sm:items-stretch max-sm:flex-col"><form className="flex flex-1 items-center gap-2 rounded-full border border-border-strong bg-surface px-3 focus-within:ring-3 focus-within:ring-ring/25" onSubmit={searchStaff}><Search aria-hidden="true" size={16} /><label className="sr-only" htmlFor="staff-search">Search staff</label><Input className="border-0 bg-transparent px-0 shadow-none focus-visible:ring-0" defaultValue={query} id="staff-search" key={query} name="search" placeholder="Search email or name" type="search" /><Button aria-label="Search" size="icon-xs" title="Search" type="submit" variant="ghost"><ArrowRight aria-hidden="true" size={15} /></Button></form><label className="flex items-center gap-2 rounded-full border border-border-strong bg-surface px-3.5"><Filter aria-hidden="true" size={16} /><span className="sr-only">Filter by status</span><select className="h-10 bg-transparent text-sm" onChange={(event) => updateFilters(query, event.target.value as typeof statusFilter)} value={statusFilter}><option value="ALL">All statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></label></div>
        <div className="mt-7 grid grid-cols-[minmax(280px,.7fr)_minmax(0,1.3fr)] items-start gap-7 max-md:grid-cols-1">
          <section className="border-r border-border pr-7 max-md:border-r-0 max-md:border-b max-md:pr-0 max-md:pb-6" aria-labelledby="educator-list-title"><div className="mb-4 flex items-end justify-between"><div><Eyebrow>Pilot staff</Eyebrow><h2 className="mt-2 text-2xl font-extrabold" id="educator-list-title">Staff</h2></div><span className="text-[13px] text-muted-foreground">{visibleStaff.length}</span></div>{visibleStaff.length === 0 ? <p className="border-t border-border py-7 text-muted-foreground">No staff match these filters.</p> : visibleStaff.map((staffMember) => { const access = data.access.find((candidate) => candidate.userId === staffMember.id); return <button className={cn("grid w-full grid-cols-[42px_minmax(0,1fr)_auto] items-center gap-3 rounded-xl px-2 py-3 text-left transition-colors hover:bg-surface-soft", selectedUserId === staffMember.id && "bg-accent hover:bg-accent")} key={staffMember.id} onClick={() => setSelectedUserId(staffMember.id)} type="button"><span className="grid size-[42px] place-items-center rounded-full border border-info-border bg-accent text-xs font-extrabold text-primary-strong">{staffMember.displayName.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span><span className="grid min-w-0 gap-1"><strong>{staffMember.displayName}</strong><small className="truncate text-xs text-muted-foreground">{staffMember.role === "ADMIN" ? "Admin / Supervisor" : "Educator"} · {staffMember.email}</small></span><span className={cn("rounded-full px-2 py-1 text-[10px] font-extrabold", access?.active ? "bg-success-soft text-success" : "bg-surface-soft text-muted-foreground")}>{access?.active ? "Active" : "Inactive"}</span></button>; })}</section>
          {selected ? <section className="rounded-2xl border border-border bg-surface p-5 shadow-card" aria-labelledby="access-editor-title">
            <header className="flex flex-wrap items-center gap-3 border-b border-border pb-5">
              <span className="grid size-12 shrink-0 place-items-center rounded-full border border-info-border bg-accent text-sm font-extrabold text-primary-strong">{selected.displayName.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span>
              <div className="min-w-0 flex-1 basis-52"><Eyebrow>{selected.role === "ADMIN" ? "Admin / Supervisor access" : "Educator access"}</Eyebrow><h2 className="mt-2 break-words text-2xl font-extrabold" id="access-editor-title">{selected.displayName}</h2><p className="mt-1 truncate text-sm text-muted-foreground">{selected.email}</p></div>
