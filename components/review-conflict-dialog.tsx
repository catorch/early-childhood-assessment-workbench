...
export function ReviewConflictDialog({
  ...
  return (
    <AlertDialog open={Boolean(attempted)} onOpenChange={(open) => !open && onUseCurrent()}>
      <AlertDialogContent className="max-h-[calc(100vh-2rem)] w-[calc(100%-1.25rem)] max-w-[560px] gap-0 overflow-y-auto rounded-2xl border border-border bg-surface p-0 shadow-pop">
        <AlertDialogHeader className="grid grid-cols-[auto_1fr] place-items-start gap-3 p-[22px] text-left max-sm:p-[18px_15px]">
          ...
          <div>
            <AlertDialogTitle className="text-[21px] font-extrabold text-ink">Review changed in another session</AlertDialogTitle>
