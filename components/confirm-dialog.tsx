...
export function ConfirmDialog({
  ...
  return (
    <AlertDialog open={open} onOpenChange={(nextOpen) => !nextOpen && !pending && onCancel()}>
      <AlertDialogContent
        className="max-h-[calc(100vh-2rem)] w-[calc(100%-1.25rem)] max-w-[520px] gap-0 overflow-y-auto rounded-2xl border border-border bg-surface p-0 shadow-pop"
      ...
      >
        <AlertDialogHeader className="grid grid-cols-[auto_1fr_auto] place-items-start gap-3 p-[22px] text-left max-sm:p-[18px_15px]">
          ...
          <div className="min-w-0">
            <AlertDialogTitle className="text-[21px] font-extrabold text-ink">{title}</AlertDialogTitle>
