...
export function AssessmentIntake() {
  ...
  return (
    <PageShell>
      ...
      <header className="max-w-[720px]"><Eyebrow>New observation</Eyebrow><h1 className="mt-2.5 font-heading text-4xl font-bold leading-tight max-sm:text-[30px]">Upload observational video</h1><p className="mt-2.5 leading-relaxed text-muted-foreground">Add one short observational video for this assessment.</p></header>
      ...
      <div className="mt-8 grid grid-cols-[minmax(0,1fr)_310px] items-start gap-12 max-md:grid-cols-1">
        <section className="border-t border-border pt-7" aria-labelledby="observation-details-title">
          ...
          <div className="mb-6 grid grid-cols-3 overflow-hidden rounded-2xl border border-border bg-surface max-sm:grid-cols-1">
          ...
          {file ? (
            ...
            <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 rounded-2xl border border-success-border bg-success-soft p-5 max-sm:grid-cols-[auto_minmax(0,1fr)]">
              <span className="grid size-11 place-items-center rounded-full bg-success text-white"><CheckCircle2 aria-hidden="true" /></span><div className="min-w-0"><Eyebrow className="bg-white/70 text-success-strong">Upload complete</Eyebrow><h3 className="mt-1 truncate text-lg font-bold">{uploadedVideo.originalFilename}</h3><p className="mt-1 text-[13px] text-muted-foreground">{(uploadedVideo.byteSize / 1024 / 1024).toFixed(1)} MB · private assessment video</p></div>
